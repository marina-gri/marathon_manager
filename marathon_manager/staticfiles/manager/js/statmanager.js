document.addEventListener('DOMContentLoaded', () => {
    ...
    const sectionContent = document.getElementById('section-content');  // Контейнер для динамического контента
    ...
    function updateHeader(selectedView) {
        sectionHeader.innerHTML = ''; // Очищаем предыдущий заголовок
    ...
    viewSelector.addEventListener('change', () => {
        ...
        sectionContent.innerHTML = ''; // Очищаем текущий контент
    ...
    function pitStat() {
        // Извлекаем данные из элементов script с типом application/json
        ...
        function findPilot(sessionNumber, teamName) {
            // Находим сессию с соответствующим номером и командой
        ...
        function renderPitStopData(pitStops) {
            ...
            scheduleBlock.innerHTML = ''; // Очищаем старые данные
        ...
        // Отображаем статистику о пит-стопах
        renderPitStopData(pitstopData);
    }

    // функция отображения статистики пилотов по сессиям
    function sessionStat() {
        // Извлекаем данные из элемента script с типом application/json
        const competitionData = JSON.parse(document.getElementById('competition-data').textContent);
        const sessionData = JSON.parse(document.getElementById('session-data').textContent);
        ...
        function renderPilotData(groupedSessions) {
            const sessionStatisticData = document.querySelector('#section-content'); // Используем #section-content
            sessionStatisticData.innerHTML = ''; // Очищаем блок перед рендером
    ...
    function sessionsSchedule() {
        ...
        function scrollToRelevantSession(teamId) {
            ...
            const teamSessions = sessionsData
                ...
                console.log('FHJKFHSKDJFHDSKJHF', teamSessions.find(session => session.active))
        ...
        function updateScheduleBlock(teamId) {
            ...
            if (teamSessions.length === 0) {
                scheduleBlock.innerHTML = '<p>Нет сессий для этой команды</p>';
                return;
            }
        ...
        function initializeScheduleOnLoad() {
            ...
            if (initialTeamId) {
                ...
                console.log('График сессий обновлен.');
                scrollToRelevantSession(teamId);
            } else {
                console.log('Не выбрана команда. Добавьте hash #team-<ID> в URL.');
            }
        }
