document.addEventListener('DOMContentLoaded', () => {
    ...
    waitForElapsedSecondsUpdate().then((initialElapsedSeconds) => {
        console.log(`Изначальное значение elapsedSeconds: ${initialElapsedSeconds}`);
        ...
        if (activePitStop) {
            ...
            console.log(`Pit elapsed: ${pitElapsed}`);
        ...
        async function handlePitButtonClick() {
            ...
            if (!activePitStop) {
                ...
                try {
                    ...
                    console.log(`Пит-стоп начат для команды ${currentTeamId} в момент времени ${currentTime}`);

                    // Завершаем текущую сессию
                    await updateSession(currentSession.id, sessionFinishData);
                    console.log(`Сессия ${sessionFinishData.session_number} завершена`);
                ...
                if (confirmation) {
                    ...
                    try {
                        ...
                        console.log(`Текущая пит`, activePitStop.id);
                        await updatePitStop(activePitStop.id ,pitStopData);
                        startStopwatch(stopwatchElement, 0); // Запускаем таймер
                        console.log(`Пит-стоп завершен для команды ${currentTeamId} в момент времени ${currentTime}`);
        ...
        function getNextSession(currentTeamId, nextSessionNumber) {
            ...
            try {
                ...
                if (nextSession) {
                    ...
                    console.log(`Сессия с номером ${nextSessionNumber} для команды ${currentTeamId} не найдена.`);
                    return false;
                }
            } catch (error) {
                console.error('Ошибка при парсинге данных сессий:', error);
        ...
        function getCurrentPitStop(currentTeamId) {
            ...
            try {
                ...
                console.log('activePitStop:', activePitStop);
